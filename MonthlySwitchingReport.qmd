...
On the 7th March, our total Active Database Size (based on the volume of customers that have placed at least one order in the last 28 days) was `r totalDBTMValue`.
...
The following table shows how the database size has changed by Loyalty Segment (Including the size of the recently lapsed customer pool as well):
...
switchingData %>%
  ...
  select(-freq) %>%
  arrange(shopLess) %>%
  select(reportPeriod, shopLess, perc) %>%
  spread(reportPeriod, perc)
...
And just filtered to customers who switched from Getir, Gorillas, Zapp and Tesco Whoosh:

```{r}
#| label: Shop More with Gopuff - Quantified Reasons - Trended - Getir Gorillas and Zapp

smt1 <- switchingData %>%
  filter(Q7.1 == 1, Q4 %in% c(4,5,17,21)) %>%
  group_by(reportPeriod) %>% summarise(n=n()) %>% mutate(Response = "Gopuff’s product prices were cheaper")%>%
  select(reportPeriod, Response, n)

smt2 <- switchingData %>%
  filter( Q7.2 == 1, Q4 %in% c(4,5,17,21)) %>%
  group_by( reportPeriod) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had better product discounts")%>%
  select(reportPeriod, Response, n)

smt3 <- switchingData %>%
  filter(Q7.3 == 1, Q4 %in% c(4,5,17,21)) %>%
  group_by( reportPeriod) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had better money off coupons")%>%
  select(reportPeriod, Response, n)

smt4 <- switchingData %>%
  filter(Q7.4 == 1, Q4 %in% c(4,5,17,21)) %>%
  group_by( reportPeriod) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had a better range of products to choose from")%>%
  select(reportPeriod, Response, n)

smt5 <- switchingData %>%
  filter(Q7.5 == 1, Q4 %in% c(4,5,17,21)) %>%
  group_by(reportPeriod) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had better in-stock product availability")%>%
  select(reportPeriod, Response, n)

smt6 <- switchingData %>%
  filter(Q7.6 == 1, Q4 %in% c(4,5,17,21)) %>%
  group_by( reportPeriod) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had lower fees, such as delivery fees")%>%
  select(reportPeriod, Response, n)

smt7 <- switchingData %>%
  filter(Q7.7 == 1, Q4 %in% c(4,5,17,21)) %>%
  group_by(reportPeriod) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had a better loyalty scheme with Puff Points")%>%
  select(reportPeriod, Response, n)

smt8 <- switchingData %>%
  filter( Q7.8 == 1, Q4 %in% c(4,5,17,21)) %>%
  group_by(reportPeriod) %>% summarise(n=n()) %>% mutate(Response = "Gopuff has a better subscription service with Fam")%>%
  select(reportPeriod, Response, n)

smt9 <- switchingData %>%
  filter( Q7.9 == 1, Q4 %in% c(4,5,17,21)) %>%
  group_by(reportPeriod) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had better delivery drivers")%>%
  select(reportPeriod, Response, n)

smt10 <- switchingData %>%
  filter( Q7.10 == 1, Q4 %in% c(4,5,17,21)) %>%
  group_by(reportPeriod) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had a better app")%>%
  select(reportPeriod, Response, n)

smt11 <- switchingData %>%
  filter( Q7.11 == 1, Q4 %in% c(4,5,17,21)) %>%
  group_by(reportPeriod) %>% summarise(n=n()) %>% mutate(Response = "Gopuff offered quicker delivery")%>%
  select(reportPeriod, Response, n)

smt12 <- switchingData %>%
  filter( Q7.12 == 1, Q4 %in% c(4,5,17,21)) %>%
  group_by( reportPeriod) %>% summarise(n=n()) %>% mutate(Response = "I moved into / back into a Gopuff Delivery Zone")%>%
  select(reportPeriod, Response, n)

smt13 <- switchingData %>%
  filter( Q7.13 == 1, Q4 %in% c(4,5,17,21)) %>%
  group_by(reportPeriod) %>% summarise(n=n()) %>% mutate(Response = "Other")%>%
  select(reportPeriod, Response, n)
...
For customers decreasing in loyalty (Decreasing their Spend vs the Previous Mont or Lapsing in total), these are the retailers customers are switching to:

```{r}
#| label: Shop More Retailers

switchingData %>%
  filter(reportPeriod == reportMonth, Q1 %in% c(5,6)) %>%
  group_by(shopMore) %>%
  summarise(n=n()) %>%
  mutate(freq = round(n/sum(n),3)) %>%
  mutate(perc = percent(freq,accuracy = 0.1)) %>%
...
sl1 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.1 == 1) %>%
  group_by(Q14.1) %>% summarise(n=n()) %>% mutate(Response = "Gopuff’s product prices were more expensive")%>%
  select(Response, n)

sl2 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.2 == 1) %>%
  group_by(Q14.2) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had weaker product discounts")%>%
  select(Response, n)

sl3 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.3 == 1) %>%
  group_by(Q14.3) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had weaker money off coupons")%>%
  select(Response, n)

sl4 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.4 == 1) %>%
  group_by(Q14.4) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had a weaker range of products to choose from")%>%
  select(Response, n)

sl5 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.5 == 1) %>%
  group_by(Q14.5) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had weaker in-stock product availability")%>%
  select(Response, n)

sl6 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.6 == 1) %>%
  group_by(Q14.6) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had higher fees, such as delivery fees")%>%
  select(Response, n)

sl7 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.7 == 1) %>%
  group_by(Q14.7) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had a weaker loyalty scheme with Puff Points")%>%
  select(Response, n)

sl8 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.8 == 1) %>%
  group_by(Q14.8) %>% summarise(n=n()) %>% mutate(Response = "Gopuff’s Fam Subscription Service wasn’t offering me value for money")%>%
  select(Response, n)

sl9 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.9 == 1) %>%
  group_by(Q14.9) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had worse delivery drivers")%>%
  select(Response, n)

sl10 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.10 == 1) %>%
  group_by(Q14.10) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had an inferior app")%>%
  select(Response, n)

sl11 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.11 == 1) %>%
  group_by(Q14.11) %>% summarise(n=n()) %>% mutate(Response = "Gopuff offered slower delivery")%>%
  select(Response, n)

sl12 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.12 == 1) %>%
  group_by(Q14.12) %>% summarise(n=n()) %>% mutate(Response = "I temporarily / permanently moved out of a Gopuff Delivery Zone")%>%
  select(Response, n)

sl13 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.13 == 1) %>%
  group_by(Q14.13) %>% summarise(n=n()) %>% mutate(Response = "Other")%>%
  select(Response, n)
...
# Create fake column to allow to cross join
totalSwitchingLessResponses$x <- 1
dataSwitchingLessReasonsTable$x <- 1

dataSwitchingLessReasonsTable %>%
  full_join(totalSwitchingLessResponses, by = "x") %>%
  select(-x) %>%
  arrange(desc(n)) %>%
  mutate(freq = round(n/total,3)) %>%
  mutate(perc = percent(freq, accuracy = 0.1)) %>%
...
And just filtered to customers who switched to Getir, Gorillas and Zapp:

```{r}
#| label: Shop Less with Gopuff - Quantified Reasons - Just qcomm

sl1 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.1 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q14.1) %>% summarise(n=n()) %>% mutate(Response = "Gopuff’s product prices were more expensive")%>%
  select(Response, n)

sl2 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.2 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q14.2) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had weaker product discounts")%>%
  select(Response, n)

sl3 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.3 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q14.3) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had weaker money off coupons")%>%
  select(Response, n)

sl4 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.4 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q14.4) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had a weaker range of products to choose from")%>%
  select(Response, n)

sl5 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.5 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q14.5) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had weaker in-stock product availability")%>%
  select(Response, n)

sl6 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.6 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q14.6) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had higher fees, such as delivery fees")%>%
  select(Response, n)

sl7 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.7 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q14.7) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had a weaker loyalty scheme with Puff Points")%>%
  select(Response, n)

sl8 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.8 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q14.8) %>% summarise(n=n()) %>% mutate(Response = "Gopuff’s Fam Subscription Service wasn’t offering me value for money")%>%
  select(Response, n)

sl9 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.9 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q14.9) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had worse delivery drivers")%>%
  select(Response, n)

sl10 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.10 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q14.10) %>% summarise(n=n()) %>% mutate(Response = "Gopuff had an inferior app")%>%
  select(Response, n)

sl11 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.11 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q14.11) %>% summarise(n=n()) %>% mutate(Response = "Gopuff offered slower delivery")%>%
  select(Response, n)

sl12 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.12 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q14.12) %>% summarise(n=n()) %>% mutate(Response = "I temporarily / permanently moved out of a Gopuff Delivery Zone")%>%
  select(Response, n)

sl13 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q14.13 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q14.13) %>% summarise(n=n()) %>% mutate(Response = "Other")%>%
  select(Response, n)

totalSwitchingLessResponses <- switchingData %>%
  filter(reportPeriod == reportMonth, Q1 %in% c(5,6), Q11 %in% c(4,5,17)) %>%
  summarise(total=n())
...
### Which orders did they reduce with Gopuff in the last month:

```{r}
#| label: Shop Less with Gopuff - Which orders

slo1 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q16.1 == 1) %>%
  group_by(Q16.1) %>% summarise(n=n()) %>% mutate(Response = "Day to Day Grocery Orders")%>%
  select(Response, n)

slo2 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q16.2 == 1) %>%
  group_by(Q16.2) %>% summarise(n=n()) %>% mutate(Response = "Drinks Orders")%>%
  select(Response, n)

slo3 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q16.3 == 1) %>%
  group_by(Q16.3) %>% summarise(n=n()) %>% mutate(Response = "Snacks Orders")%>%
  select(Response, n)

slo4 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q16.4 == 1) %>%
  group_by(Q16.4) %>% summarise(n=n()) %>% mutate(Response = "Smoking Orders")%>%
  select(Response, n)

slo5 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q16.5 == 1) %>%
  group_by(Q16.5) %>% summarise(n=n()) %>% mutate(Response = "Urgent Needs Orders")%>%
  select(Response, n)
...
# Create fake column to allow to cross join
totalSwitchingLessResponses$x <- 1
dataSwitchingLessOrderTypeTable$x <- 1
...
```{r}
#| label: Shop Less with Gopuff - Which orders - just qcomm

slo1 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q16.1 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q16.1) %>% summarise(n=n()) %>% mutate(Response = "Day to Day Grocery Orders")%>%
  select(Response, n)

slo2 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q16.2 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q16.2) %>% summarise(n=n()) %>% mutate(Response = "Drinks Orders")%>%
  select(Response, n)

slo3 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q16.3 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q16.3) %>% summarise(n=n()) %>% mutate(Response = "Snacks Orders")%>%
  select(Response, n)

slo4 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q16.4 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q16.4) %>% summarise(n=n()) %>% mutate(Response = "Smoking Orders")%>%
  select(Response, n)

slo5 <- switchingData %>%
  filter(reportPeriod == reportMonth, Q16.5 == 1, Q11 %in% c(4,5,17)) %>%
  group_by(Q16.5) %>% summarise(n=n()) %>% mutate(Response = "Urgent Needs Orders")%>%
  select(Response, n)
